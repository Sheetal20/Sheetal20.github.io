...
try{
	...
	$chck= mysqli_query($conn,"select * from complain_type where type_name='".$type_name."' limit 1");
    if(mysqli_fetch_row($chck)) {
        $_SESSION['chckmessage']="This complain_type already exists.";
			session_write_close();
		    header('location:complain_type.php');
    exit;
      }
	  else{
	$sql="insert into complain_type(type_name) values('".$type_name."')";
...
<li><a href="#" style="color:#990026"><strong>COMPLAIN TYPE DETAILS</strong></a></li>
...
<div class="panel-heading" style="font-family:georgia;color:black;background-color:#ffec80;"><div class="text-center"><strong>COMPLAIN TYPE</strong></div></div>
...
<label for="type_name">ENTER COMPLAIN TYPE</label>
<input type="text" name="type_name" id="type_name" class="form-control" placeholder="Complain Type" required="required">
</div>
<div class="form-group">
...
<td class="text-center" ><button class="btn btn-success" data-toggle="modal" data-target="#editModal<?php echo $tb['type_id']; ?>">Edit</button></td>
<td class="text-center" ><a class="btn btn-danger" href="delete_complain_type.php?id=<?php echo $tb['type_id']; ?>">Delete</a></td>
</tr>

<div class="modal fade" id="editModal<?php echo $tb['type_id']; ?>" role="dialog">
...
<form class="form-inline" role="form" method="post" action="update_complain_type.php">
<div class="container center_div">
<div class="form-group">
<label for="type_name">ENTER COMPLAIN TYPE</label>
<?php
$sql1="select type_name from complain_type where type_id ='".$tb['type_id']."'";
$query1=mysqli_query($conn,$sql1);
foreach ($query1 as $dm){
?>
<input type="text" class="form-control" name="type_name" id="type_name" value="<?php echo $dm['type_name'];?>"> 
</div>
<input type="hidden" name="type_id" value="<?php echo $tb['type_id'];?>">
